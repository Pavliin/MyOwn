...
- **Jellyfin** (bibliothèque films/musique) : déploiement nu **fait** (chart officiel `jellyfin/jellyfin-helm`, un vrai bug d'`inotify` hôte trouvé et corrigé au premier déploiement — `notes-techniques.md`) + intégration SSO Authentik (plugin communautaire tiers, même schéma d'intégration que le `user_oidc` de Nextcloud, à valider) + sauvegarde Restic (base + bibliothèque), ces deux derniers encore à faire. Bibliothèque de test uniquement à ce stade — l'usage réel visé (partager des films/musiques achetés avec la famille, dispersée sur plus de 800 km) dépend d'une exposition internet réelle, différé en Phase 4
...
- **Pi-hole/AdGuard Home** : blocage de trackers/pub au niveau DNS pour tout le réseau familial. Ne dépend pas du nom de domaine (fonctionne en LAN pur), mais du mini PC comme hôte stable plutôt que du cluster de dev — celui-ci a déjà été recréé plusieurs fois pendant le développement (Phase 3.5, pression disque/remapping de ports), ce qui couperait le DNS de toute la maison si Pi-hole y tournait comme un service normal. Garde-fou non négociable, pas une amélioration optionnelle : DNS secondaire de repli (1.1.1.1/9.9.9.9) configuré au niveau du DHCP du routeur, pour qu'une panne de Pi-hole dégrade le blocage de pub plutôt que de couper l'accès internet de toute la maison. Placement à trancher à l'implémentation (service k3s classique, ou hors cluster comme WireGuard — même question de dépendance circulaire à évaluer : le DNS de toute la maison doit-il dépendre de la santé du cluster ?).
