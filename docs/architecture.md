...
- **GitOps via ArgoCD** : déploiement piloté par ce dépôt Git, cohérent avec la pratique professionnelle de l'auteur. **ArgoCD n'est pas l'orchestrateur** — k3s l'est, et continue de faire tourner tous les services indépendamment d'ArgoCD ; ArgoCD ne fait que réconcilier l'état du cluster avec git, rien ne dépend de lui à l'exécution. Chaque déploiement MyOwn a sa propre instance ArgoCD, locale à son propre cluster — aucun mécanisme central ne peut ni ne doit déclencher une synchronisation sur l'installation de quelqu'un d'autre (cohérent avec le principe déjà posé dans `vision-long-terme.md` : chaque installation reste souveraine, pas d'opérateur central unique). Les 13 `Application` enfants restent en synchronisation automatique (`selfHeal: true`) — un `git push` sur `master` se propage au cluster de dev en quelques minutes. **Décidé mais pas encore implémenté** : à terme, distinction entre services sensibles (basculeraient en synchronisation manuelle, déclenchée par l'admin de l'installation concernée, précédée d'une annonce — cf. ci-dessous) et services d'infrastructure invisibles pour la famille (peuvent rester automatiques sans risque) :
  - **Sensibles** (impact direct sur l'usage familial) : Authentik (SSO, point d'entrée unique — une régression y bloque l'accès à tout le reste), Vaultwarden, Nextcloud, Immich, Tuwunel, LiveKit, Mailu, Jellyfin, Ollama (une fois la Phase 5 réelle).
  - **Sûrs, restent automatiques** : `monitoring` (Prometheus/Grafana, admin uniquement), `uptime-kuma`, `gandi-dyndns`, `livekit-turn-cert-sync`, `mailu-cert-sync` (outils de supervision/infra, invisibles pour la famille).
  ...
  - **`root` (l'app-of-apps) fait exception, et reste volontairement en synchronisation manuelle dès aujourd'hui** — pas seulement pour les installations en usage familial réel. Ancienne raison documentée ici ("doit rester automatique pour continuer à découvrir les nouvelles `Application`") invalidée par la pratique réelle : `root` est aussi ce qui répercute un `git push` sur `master` vers le `spec` de chaque `Application` enfant (sources, `targetRevision`) — s'il resynchronisait tout seul, il écraserait silencieusement tout test réalisé en direct sur un enfant avant que le correctif ne soit committé (heurté pour de vrai au moins deux fois, 2026-08-15 et 2026-08-22, voir `notes-techniques.md`). Sur une installation qui suit un tag de release épinglé (mini PC), un `argocd app sync root` explicite fait de toute façon déjà partie du cycle de release à chaque fois (`scripts/pin-release.sh`) — la découverte automatique de nouvelles `Application` n'a en pratique jamais été utile sans cette étape manuelle.
...
- **Pi-hole** (Phase 4) — DNS filtrant (blocage pub/trackers) pour tout le réseau familial. Même famille de raisonnement de dépendance circulaire que le VPN et le watchdog ci-dessus, mais l'enjeu est différent : ce n'est pas l'accès au cluster qui est en jeu, c'est le DNS de toute la maison — le cluster de dev a déjà été recréé plusieurs fois pendant le développement (Phase 3.5, pression disque/remapping de ports), ce qui aurait coupé le DNS du foyer entier si Pi-hole y avait tourné comme un `Deployment` k8s normal. Conteneur Docker sur l'hôte (`pihole-myown`, `scripts/pihole-setup.sh`), volontairement hors GitOps et hors k3s — nouvelle dépendance Docker sur le mini PC, indépendante du containerd embarqué de k3s. Garde-fou non négociable : DNS secondaire de repli (1.1.1.1) configuré au niveau du DHCP du routeur, pour qu'une panne de Pi-hole dégrade le blocage de pub plutôt que de couper l'accès internet de toute la maison — validé avec un vrai test (coupure du conteneur, bascule confirmée en moins d'une seconde). Overrides DNS locaux pour `*.offsystem.fr` (résolution directe vers l'IP LAN du mini PC) : même logique de split-horizon que `coredns-custom.yaml` côté cluster, cette fois côté réseau domestique — contourne la limite de hairpin NAT UDP de la Freebox déjà rencontrée avec LiveKit. Pas de sauvegarde Restic dédiée : contenu quasi entièrement reproductible (config recréée à l'identique par le script, liste de blocage publique retéléchargée automatiquement) — seule perte potentielle serait une éventuelle liste blanche/noire ajoutée manuellement dans le futur, acceptée comme limitation connue plutôt qu'un pipeline de sauvegarde pour une valeur quasi nulle.
