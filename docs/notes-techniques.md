...
`redirect_uris` utilise `matching_mode: regex` plutôt que `strict` — la documentation de Vaultwarden signale des cas où le port est absent de l'URL de callback qu'il envoie lui-même sur un port non standard ; le regex tolère les deux formes plutôt que de parier sur laquelle il enverra réellement.

**Découverte automatique des blueprints montés** : fonctionne (le mécanisme est réel, testé), mais pas instantanément — après le montage du nouveau Secret, il a fallu déclencher manuellement la tâche `blueprints_discovery` (`ak shell`) pour qu'elle enregistre et applique le blueprint immédiatement, plutôt que d'attendre son prochain passage périodique. Sur un déploiement réel non observé en direct, ça finit par s'appliquer tout seul — juste pas nécessairement dans la minute qui suit un déploiement, à garder en tête si un test semble "ne pas marcher" juste après un merge.
