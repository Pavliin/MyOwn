...
- **Roadmap Phase 2 ("Nextcloud + Immich")**: Nextcloud side complete — deployed (official `nextcloud/helm` chart, Postgres+Redis subcharts), SSO-integrated with Authentik and validated end-to-end with a real browser login (official `user_oidc` app via the chart's `hooks.before-starting`; HTTPS on `:8453` since `user_oidc` refuses plain HTTP; also fixed Nextcloud's SSRF guard and a Redis-session password encoding bug — five real incidents total, all in `notes-techniques.md`), Calendar/Contacts apps installed, Restic backup extended (files PVC + `pg_dump` of Postgres in the same snapshot, since calendar/contacts/shares live in the DB, not as files) and validated with a real backup+restore test. Immich deployed bare (official `immich/immich` chart; Postgres is a hand-written StatefulSet on `ghcr.io/immich-app/postgres` since the Bitnami postgresql chart used elsewhere isn't compatible with that image — see `notes-techniques.md`). No SSO/Restic/Android for Immich yet. Nextcloud's native Photos app is still enabled — disable it (`occ app:disable photos`) once Immich is confirmed working end-to-end, per the "one app per use case" principle in architecture.md §2.
- **Not yet touched**: Mailcow, Ollama, Conduwuit/LiveKit.

**Next step**: validate Immich end-to-end in the browser (account creation, photo upload), then disable Nextcloud's `photos` app in a quick follow-up — that closes out Phase 2 on the dev cluster except SSO/Restic/Android for Immich, which can come later or start Phase 3, ask before assuming which.
