...
- **Roadmap Phase 2 ("Nextcloud + Immich")**: in progress — Nextcloud deployed (official `nextcloud/helm` chart, Postgres+Redis subcharts) and SSO-integrated with Authentik, validated end-to-end with a real browser login (official `user_oidc` app, installed/registered via the chart's `hooks.before-starting` mechanism, idempotent on every pod start; HTTPS on `:8453` since `user_oidc` refuses plain HTTP; Nextcloud's SSRF guard and a Redis-session password encoding bug both had to be fixed too — five real incidents total, all in `notes-techniques.md`). Still open: Restic backup extension, Android app, Immich. Immich not started — its official chart needs a separately-provisioned Postgres with the `pgvector` extension, deserves its own research pass.
- **Not yet touched**: Immich, Mailcow, Ollama, Conduwuit/LiveKit.

**Next step**: continue Phase 2 — extend the Restic backup pipeline to Nextcloud, then tackle Immich. No fixed order forced yet — ask before assuming which comes first.
