...
- **Roadmap Phase 2 ("Nextcloud + Immich")**: in progress — Nextcloud deployed (official `nextcloud/helm` chart, Postgres+Redis subcharts) and SSO-integrated with Authentik (official `user_oidc` app, installed/registered via the chart's `hooks.before-starting` mechanism, idempotent on every pod start — see `notes-techniques.md`). SSO required moving Nextcloud to HTTPS on `:8453` (same mkcert pattern as Vaultwarden — `user_oidc` refuses to run over HTTP, unrelated to Vaultwarden's own HTTPS reason). Still open: Restic backup extension, Android app, Immich. Immich not started — its official chart needs a separately-provisioned Postgres with the `pgvector` extension, deserves its own research pass.
