...
- **Roadmap Phase 2 ("Nextcloud + Immich")**: started — Nextcloud deployed bare (official `nextcloud/helm` chart, Postgres+Redis subcharts, no SSO/Restic/Android yet — those are separate follow-up increments, same incremental pattern as Phase 1). See `notes-techniques.md` for a real chart-source gotcha found before deploying (the chart's own `-db` Secret bakes the Postgres password in as plaintext at template time, bypassing `existingSecret` — worked around via `externalDatabase.existingSecret` instead). Immich not started — its official chart needs a separately-provisioned Postgres with the `pgvector` extension, deserves its own research pass.
- **Not yet touched**: Immich, Mailcow, Ollama, Conduwuit/LiveKit.

**Next step**: continue Phase 2 — SSO-integrate Nextcloud with Authentik (mirrors the Vaultwarden blueprint pattern), extend the Restic backup pipeline to it, then tackle Immich. No fixed order forced yet — ask before assuming which comes first.
